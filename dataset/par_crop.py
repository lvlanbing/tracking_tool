...
dataset_path = '/media/upup/d6c7e0e6-efe4-4f7b-9f42-bfcc4fe51fee/lv_dataset/LSOTB-TIR/Training_Dataset/LSOTB-TIR_TrainingData/TrainingData/'
sub_sets = ['TIR_training_001', 'TIR_training_002', 'TIR_training_003', 'TIR_training_004',]

# dataset_path = '/media/upup/d6c7e0e6-efe4-4f7b-9f42-bfcc4fe51fee/lv_dataset/GOT/train_data/train'
...
def crop_video(video, d_set, crop_path, instanc_size):
    if video != 'list.txt':
        ...
        for idx, gt_line in enumerate(groundtruth):
            ...
            bbox = [int(float(gt_image[0])),int(float(gt_image[1])),int(float(gt_image[2])),int(float(gt_image[3]))]#xmin,ymin,xmax,ymax
